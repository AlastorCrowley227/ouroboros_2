...
def test_llm_chat_sets_num_ctx_option(monkeypatch):
    ...
    monkeypatch.setenv("OLLAMA_PREFER_NATIVE_API", "1")
...
def test_llm_chat_payload_serialization_fallback(monkeypatch):
    captured = {}
    ...
    msg, _ = llm.chat(messages=[{"role": "user", "content": Weird()}], model="llama3.2:latest")
    assert msg["content"] == "ok"
    assert captured["payload"]["messages"][0]["content"] == "weird"
